  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
            ...
            SET "firstName" = split_part("name", ' ', 1),
                "lastName" = COALESCE(NULLIF(substring("name" from position(' ' in "name") + 1), ''), 'Krew')
            WHERE "name" IS NOT NULL
        `);

    // Set fallbacks for any users that might have had a null name
    await queryRunner.query(
      `UPDATE "users" SET "firstName" = 'Friendly' WHERE "firstName" IS NULL OR "firstName" = ''`,
    );
    await queryRunner.query(
      `UPDATE "users" SET "lastName" = 'User' WHERE "lastName" IS NULL OR "lastName" = ''`,
    );
