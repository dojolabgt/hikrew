...
export class RecurrenteBlendService {
    ...
    constructor(private readonly configService: ConfigService) {
        this.publicKey = this.configService.getOrThrow<string>('BLEND_RECURRENTE_PUBLIC_KEY');
        this.secretKey = this.configService.getOrThrow<string>('BLEND_RECURRENTE_SECRET_KEY');
        this.monthlyPriceCents =
            this.configService.get<number>('BILLING_PRO_MONTHLY_CENTS') ?? DEFAULT_MONTHLY_CENTS;
        this.yearlyPriceCents =
            this.configService.get<number>('BILLING_PRO_YEARLY_CENTS') ?? DEFAULT_YEARLY_CENTS;
    }
    ...
    async createSubscriptionCheckout(
        ...
        const payload = {
            items: [
                {
                    name: itemName,
                    description: 'Acceso completo a Blend: clientes ilimitados, cotizaciones ilimitadas y más.',
                    currency: 'GTQ',
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new InternalServerErrorException('No se pudo crear el checkout de suscripción');
            }
    ...
    async cancelSubscription(subscriptionId: string): Promise<void> {
        try {
            const res = await fetch(`${RECURRENTE_API}/subscriptions/${subscriptionId}`, {
                method: 'DELETE',
                headers: this.authHeaders,
            });
            ...
            if (error instanceof InternalServerErrorException) throw error;
            this.logger.error(`Network error cancelling subscription ${subscriptionId}`, error);
            throw new InternalServerErrorException('No se pudo cancelar la suscripción');
        }
    }
