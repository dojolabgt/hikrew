...
export class ClientsService {
    ...
    async create(workspaceId: string, dto: CreateClientDto): Promise<Client> {
        ...
        if (workspace?.plan === WorkspacePlan.FREE) {
            ...
            if (count >= 5) {
                throw new BadRequestException('Has alcanzado el límite de 5 clientes para el plan gratuito. Mejora tu plan para agregar más.');
            }
        }
    ...
    async update(workspaceId: string, id: string, dto: UpdateClientDto): Promise<Client> {
